...
export function useCustomGlassMaterial({
  ...
  // Cache for scene objects to avoid repeated traversals
  ...
  useEffect(() => {
    sceneCache.current.needsUpdate = true;
  }, [scene]);
...
void main() {
    ...
    // Apply reflections with base amount on all surfaces
    ...
    if (enableReflections) {
        ...
        // Base reflection amount for center/flat surfaces (adjust this value: 0.0 to 1.0)
        float baseReflection = 0.5;
        
        // Calculate specular highlights specifically for edge reflections
        if (edgeMask > 0.0 && enableLighting) {
            // Point lights
            for (int i = 0; i < ` +
      maxLights +
      `; i++) {
      ...
      `; i++) {
            ...
            // Mix cubemap reflection with light specular at edges
            reflection = mix(reflection, reflection + edgeSpecular * 2.0, edgeMask);
        }
        
        // Combine base reflection with edge-enhanced reflection
        float totalReflectionStrength = baseReflection + edgeMask * edgeReflectionIntensity;
        reflection *= totalReflectionStrength;
    }
...
`;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        emissive: {
          value: [
            defaultEmissiveColorObj.r,
            defaultEmissiveColorObj.g,
            defaultEmissiveColorObj.b,
          ],
        },
  ...
  useEffect(() => {
    if (glassMaterial && glassMaterial.uniforms) {
      ...
      if (glassMaterial.uniforms.emissive) {
        glassMaterial.uniforms.emissive.value = [
          emissiveObj.r,
          emissiveObj.g,
          emissiveObj.b,
        ];
  ...
  // Function to update scene cache
  const updateSceneCache = () => {
    if (!sceneCache.current.needsUpdate) return;
    ...
    sceneCache.current = {
      glassMeshes: glassMeshesByLayer,
      lights: { pointLights, directionalLights },
      excludedObjects,
      needsUpdate: false,
  ...
  useFrame((state) => {
    ...
    // Update cache if needed (only once when scene changes)
    updateSceneCache();
    ...
    if (glassMaterial && glassMaterial.uniforms) {
      ...
      // Update light uniforms from cached lights (every frame, but no traversal)
      if (enableLighting && sceneCache.current.lights) {
        const { pointLights, directionalLights } = sceneCache.current.lights;
      ...
      // Only render transmission texture every 5 frames
      ...
      // Use cached data instead of traversing
      const glassMeshesByLayer = sceneCache.current.glassMeshes;
      const excludedObjects = sceneCache.current.excludedObjects;

      if (!glassMeshesByLayer) return;
  ...
  }, [transmissionRenderTarget, depthRenderTarget]);
